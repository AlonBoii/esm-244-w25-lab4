...
```{r}
hdi_data_raw <- read.csv(here("data", "hdi_clean.csv"))
```
...
# given SD, square to get variance, then divided by total variance to get percent variance
sd_vec <- hdi_pca$sdev
var_vec <- sd_vec^2
...
ggplot(pct_expl_df,aes(x = pc, y = pct_v)) +
  geom_col() +
  scale_y_continuous(labels = scales::percent, expand=(c(0,0))) + # makes bars go all the way down to the x = 0
  labs( x = "prinicipal component",
